...
void neural_network_import_weights(neural_network_t * network, const char * filename)
{
    FILE * file;
    if((file = fopen(filename, "r")) == NULL)
    {
        printf("Error opening file for reading\n");
        return neural_network_random_weights(network);
    }
    int i, j;
    for (i = 0; i < MNIST_LABELS; i++) {
        fscanf(file,"%f",&(network->b[i]));

        for (j = 0; j < MNIST_IMAGE_SIZE; j++) {
            fscanf(file,"%f",&(network->W[i][j]));
        }
    }
    fclose(file);
}

void neural_network_save_weights(neural_network_t * network, const char * filename)
{
...
/**
 * Feed forward propagation through the neural network
 * 
 */
...
/**
 * Run the neural network on a single image and return the label with the
 * highest activation.
 */
uint8_t neural_network_predict(mnist_image_t * image, neural_network_t * network, float *precision)
{
    float activations[MNIST_LABELS];
    int i, max;
    ...
    for (i = 1, max = 0; i < MNIST_LABELS; i++) {
        if (activations[i] > activations[max]) {
            max = i;
        }
    }
    *precision = activations[max];
    return max;
}
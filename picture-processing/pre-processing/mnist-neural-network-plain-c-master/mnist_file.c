...
uint8_t * get_labels_test(uint32_t * number_of_labels)
{
    uint8_t * labels;
    *number_of_labels = 1;
...
mnist_image_t * get_images_test(const char * path)
{
    FILE * stream;
    ...
    images = malloc(sizeof(mnist_image_t));
...
mnist_dataset_t * mnist_get_dataset_test(const char * image_path)
{
    mnist_dataset_t * dataset;
    uint32_t number_of_images = 1;
    uint32_t number_of_labels = 1;
    ...
    dataset->images = get_images_test(image_path);
    ...
    dataset->labels = get_labels_test(&number_of_labels);
